    ...
    (set, get) => ({
      ...
      fetchEpicQuests: async (forceRefresh = false) => {
        ...
        const lastFetchTime = state.lastFetch
          ? new Date(state.lastFetch).getTime()
          : 0;

        // Skip fetch if data is fresh and not forcing refresh
        if (
          !forceRefresh &&
          state.epicQuests.length > 0 &&
        ...
        try {
          ...
          if (
            !currentState.activeRoadmap.epicQuestId &&
            epicQuests.length > 0
          ) {
            // Only set active roadmap if there's an epic quest with a ready roadmap
            const questWithRoadmap = epicQuests.find(
              (quest) => quest.roadmapStatus === 'ready'
            );
            if (questWithRoadmap) {
              try {
                ...
                console.warn(
                  'Failed to set active roadmap for quest:',
                  questWithRoadmap.questId,
                  error
                );
              }
            } else {
      ...
      updateEpicQuest: async (questId, updateData) => {
        set({ isLoading: true, error: null });
        try {
          const updatedQuest = await epicQuestsApi.updateEpicQuest(
            questId,
            updateData
          );
          set((state) => ({
            ...
            activeRoadmap:
              state.activeRoadmap.epicQuestId === questId
        ...
        try {
          ...
          set((state) => ({
            ...
            roadmapCache: Object.fromEntries(
              Object.entries(state.roadmapCache).filter(
                ([key]) => key !== questId
              )
            ),
            isLoading: false,
      ...
      fetchDailyQuests: async (date, forceRefresh = false) => {
        ...
        try {
          ...
          const currentTaskIds = state.taskAccess.todayTasks
            .map((t) => t.questId)
            .sort();
          const newTaskIds = dailyQuests.map((t) => t.questId).sort();
          const hasChanged =
            JSON.stringify(currentTaskIds) !== JSON.stringify(newTaskIds) ||
            forceRefresh ||
            dailyQuests.some((task) => {
              const existing = state.taskAccess.todayTasks.find(
                (t) => t.questId === task.questId
              );
              return (
                !existing ||
      ...
      updateDailyQuest: async (questId, updateData) => {
        set({ isLoading: true, error: null });
        try {
          const updatedQuest = await dailyQuestsApi.updateDailyQuest(
            questId,
            updateData
          );
          set((state) => ({
      ...
      deleteDailyQuest: async (questId) => {
        ...
        try {
          ...
          set((state) => ({
            ...
            taskAccess: {
              ...state.taskAccess,
              todayTasks: state.taskAccess.todayTasks.filter(
                (quest) => quest.questId !== questId
              ),
              futureTasks: state.taskAccess.futureTasks.filter(
                (quest) => quest.questId !== questId
              ),
            },
            isLoading: false,
      ...
      fetchRoadmap: async (epicQuestId, forceRefresh = false) => {
        ...
        if (
          !forceRefresh &&
          cached &&
          now - cached.lastFetched < state.cacheExpiryTime
        ) {
        ...
        try {
          ...
          const epicQuest =
            state.epicQuests.find((quest) => quest.questId === epicQuestId) ||
            null;
      ...
      setActiveRoadmap: async (epicQuestId) => {
        const state = get();
        const epicQuest = state.epicQuests.find(
          (quest) => quest.questId === epicQuestId
        );
      ...
      checkTaskAccessRules: () => {
        ...
        const completionRate =
          state.taskAccess.todayTasks.length > 0
            ? state.taskAccess.todayTasks.filter(
                (task) => task.status === 'completed'
              ).length / state.taskAccess.todayTasks.length
            : 0;
      ...
      getAvailableTasks: () => {
        ...
        return {
          ...
          future: state.taskAccess.canAccessFuture
            ? state.taskAccess.futureTasks
            : [],
...
export const useActiveRoadmap = () =>
  useQuestStore((state) => state.activeRoadmap);
export const useActiveMilestoneTasks = () =>
  useQuestStore((state) => state.taskAccess.todayTasks);
export const useTodayTasks = () =>
  useQuestStore((state) => state.taskAccess.todayTasks);
export const useIsLoading = () => useQuestStore((state) => state.isLoading);
export const useIsRefreshing = () =>
  useQuestStore((state) => state.isRefreshing);
export const useQuestError = () => useQuestStore((state) => state.error);

// Individual action hooks to prevent infinite re-renders
export const useFetchEpicQuests = () =>
  useQuestStore((state) => state.fetchEpicQuests);
export const useCreateEpicQuest = () =>
  useQuestStore((state) => state.createEpicQuest);
export const useDeleteEpicQuest = () =>
  useQuestStore((state) => state.deleteEpicQuest);
export const useGenerateRoadmap = () =>
  useQuestStore((state) => state.generateRoadmap);
export const useFetchRoadmap = () =>
  useQuestStore((state) => state.fetchRoadmap);
export const useRefreshTodayData = () =>
  useQuestStore((state) => state.refreshTodayData);
export const useRefreshQuestsData = () =>
  useQuestStore((state) => state.refreshQuestsData);
export const useRefreshAllPages = () =>
  useQuestStore((state) => state.refreshAllPages);

// Daily Quest action hooks
export const useFetchDailyQuests = () =>
  useQuestStore((state) => state.fetchDailyQuests);
export const useCreateDailyQuest = () =>
  useQuestStore((state) => state.createDailyQuest);
export const useUpdateDailyQuest = () =>
  useQuestStore((state) => state.updateDailyQuest);
export const useDeleteDailyQuest = () =>
  useQuestStore((state) => state.deleteDailyQuest);
